					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateLuggage()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div id="app">
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Luggages = true">Add</base-button>
								              <modal :show.sync="modal2Luggages">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Luggage Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="LuggageId" type="text" placeholder="Enter LuggageId" v-model="luggageDetails.luggageId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Weight</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Weight" type="text" placeholder="Enter Weight" v-model="luggageDetails.weight"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Dimensions</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Dimensions" type="text" placeholder="Enter Dimensions" v-model="luggageDetails.dimensions"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateLuggage() {
      ...
      const jsonData = JSON.stringify(this.luggageDetails);
      const res = await LuggageService.update(jsonData);
...
//        this.getAllLuggages();
    ...
    async getLuggageDetails() {
      try {
			let response = await LuggageService.get(this.luggageId);
			this.luggageDetails = response.data;
